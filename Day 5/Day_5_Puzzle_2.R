...
v_inds <- which(start_mat[,1] == end_mat[,1])
h_inds <- which(start_mat[,2] == end_mat[,2])
d_inds <- setdiff(1:500,c(v_inds,h_inds))

h_lines <- cbind(start_mat[h_inds,],end_mat[h_inds,])
v_lines <- cbind(start_mat[v_inds,],end_mat[v_inds,])
d_lines <- cbind(start_mat[d_inds,],end_mat[d_inds,])
...
d_coords <- NULL
for(i in 1:nrow(d_lines)){
	d_coords <- rbind(d_coords,cbind(d_lines[i,1]:d_lines[i,3],d_lines[i,2]:d_lines[i,4]))
}


coords <- rbind(h_coords,v_coords,d_coords)
